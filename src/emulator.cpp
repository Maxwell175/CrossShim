...
uint64_t HleManager::get_stub_address(const std::string &name) {
    ...
    if (!has_hle(name)) {
        ...
        if (name == "strtod" || name == "strtof" || name == "difftime") {
            EMU_LOG << "[HLE] WARNING: No HLE callback for '" << name << "'" << std::endl;
        }
        return 0;
    }
